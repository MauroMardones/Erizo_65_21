---
title: "DIAGNÓSTICO DE MODELOS DE EVALUACIÓN DE STOCK DE ERIZO - MODELO ALTERNATIVO: USO DE ÍNDICE DE BIOMASA ESTIMADA POR MÉTODO CATDIN "
toc: TRUE
output: 
    pdf_document:
header-includes:
...
# Zona X Norte
data1        <- lisread("MAETXNa.dat") 
names(data1) <- str_trim(names(data1), side="right")
dat1         <- data1
rep1         <- reptoRlist("MAETXNa.rep")
std1         <- read.table("MAETXNa.std",header=T,sep="",na="NA",fill=T) 

# Zona X Sur
data2        <- lisread("MAETXSa.dat") 
names(data2) <- str_trim(names(data2), side="right")
dat2         <- data2
rep2         <- reptoRlist("MAETXSa.rep")
std2         <- read.table("MAETXSa.std",header=T,sep="",na="NA",fill=T) 

# Zona XI
data3        <- lisread("MAETXIa.dat") 
names(data3) <- str_trim(names(data3), side="right")
dat3         <- data3
rep3         <- reptoRlist("MAETXIa.rep")
std3         <- read.table("MAETXIa.std",header=T,sep="",na="NA",fill=T) 
...
```{r RetrospectivoXNalt, echo=F, warning=FALSE,eval=F,include=F}
source(paste(dir.fun,"Fn_Retrospectivo.R",sep=""))
Retrospectivo("MAETXNa",dir.0,dir.1,"/RetrospectivoXN_alt") 

```


```{r RetrospectivoXSalt, echo=F, warning=FALSE,eval=F,include=F}
source(paste(dir.fun,"Fn_Retrospectivo.R",sep=""))
Retrospectivo("MAETXSa",dir.0,dir.1,"/RetrospectivoXS_alt") 

```


```{r RetrospectivoXIalt, echo=F, warning=FALSE,eval=F,include=F}
source(paste(dir.fun,"Fn_Retrospectivo.R",sep=""))
Retrospectivo("MAETXIa",dir.0,dir.1,"/RetrospectivoXI_alt") 

```


```{r VerosimilitudXNalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Verosimilitud.R",sep=""))
logRo    <- 3.17467674069
priorRo <- seq(1.9,4.1,0.1)
Verosimilitud("MAETXNa",dir.0,dir.1,"/VerosimilitudXN_alt",logRo,priorRo) 

```


```{r VerosimilitudXSalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Verosimilitud.R",sep=""))
logRo    <- 4.58827404901
priorRo <- seq(4,9,0.2)
Verosimilitud("MAETXSa",dir.0,dir.1,"/VerosimilitudXS_alt",logRo,priorRo) 

```


```{r VerosimilitudXIalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Verosimilitud.R",sep=""))
logRo    <- 4.89072430364
priorRo <- seq(4,6,0.1)
Verosimilitud("MAETXIa",dir.0,dir.1,"/VerosimilitudXI_alt",logRo,priorRo) 

```

```{r MortalidadNaturalXNalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_MortalidadNatural.R",sep=""))
rango_M   <-c(0.2, 0.21, 0.22, 0.28)
MortalidadNatural("MAETXNa",dir.0,dir.1,"/MortalidadNatural_XN_alt",rango_M) 
```

```{r MortalidadNaturalXSalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_MortalidadNatural.R",sep=""))
rango_M   <-c(0.2, 0.21, 0.22, 0.28)
MortalidadNatural("MAETXSa",dir.0,dir.1,"/MortalidadNatural_XS_alt",rango_M) 
```

```{r MortalidadNaturalXIalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_MortalidadNatural.R",sep=""))
rango_M   <-c(0.2, 0.21, 0.22, 0.28)
MortalidadNatural("MAETXIa",dir.0,dir.1,"/MortalidadNatural_XI_alt",rango_M) 
```

```{r Rango_Loo_XNalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Rango_Loo.R",sep=""))
rango_Loo   <-seq(116, 128, 0.5)

Rango_Loo("MAETXNa",dir.0,dir.1,"/RangoLoo_XN_alt",rango_Loo) 
```

```{r Rango_Loo_XSalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Rango_Loo.R",sep=""))
rango_Loo   <-seq(116, 128, 0.5)
Rango_Loo("MAETXSa",dir.0,dir.1,"/RangoLoo_XS_alt",rango_Loo) 
```

```{r Rango_Loo_XIalt, echo=F, warning=F,eval=F,include=F}
source(paste(dir.fun,"Fn_Rango_Loo.R",sep=""))
rango_Loo   <-seq(128, 136, 0.5)
Rango_Loo("MAETXIa",dir.0,dir.1,"/RangoLoo_XI_alt",rango_Loo) 
```

```{r funcionPbr_Xnortealt,echo=FALSE, message=FALSE, warning=FALSE, include=F,eval=T}
...
```{r funcionPbr_Xsuralt,echo=FALSE, message=FALSE, warning=FALSE, include=F,eval=T}
...
```{r funcionPbr_XIalt,echo=FALSE, message=FALSE, warning=FALSE, include=F,eval=T}
...
## 1.1. Ajustes y residuos - Modelo alternativo
...
## 1.2. Análisis retrospectivo de erizo zona X norte - Modelo alternativo

```{r VariablesXN_alt_std, echo=F,message=FALSE, warning=FALSE, include=T}
...
```{r Dat_RetrospectivoXN_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RetrospectivoXN_alt",sep="")
setwd(dir)
admb<-"MAETXNa"
...
```{r Fig_RetrospectivoXN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Patrón retrospectivo estándar (panel izquierdo) y  relativo (panel derecho)", fig.height=6.5, fig.width=6}
...
## 1.3. Perfil de verosimilitud de erizo zona X norte - Modelo alternativa


```{r Fig_VerosimilitudXN_alt,echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Perfil de verosimilitud erizo zona X norte", fig.height=5, fig.width=7}

source(paste(dir.fun,"functions.R",sep=""))
dir       <-paste(dir.0,"/VerosimilitudXN_alt",sep="")
setwd(dir)
...
for(i in 1:casos){
report      <- reptoRlist(paste(dir,"//MAETXNas",i,".rep",sep=""))
logRo[i]    <- report$Ro
...
names<-c("Ro","Biomasa","Desembarques","Prop_Flota","devRo","log_F","Total")
# Tabla verosimilitud
...
fig1<-ggplot() +
      geom_line(aes(x = TLk2$Ro, y = TLk2$Biomasa, colour=names[2])) +
      ...
      xlab("Ro") + 
      ylab("L-min(L)") + 
     ggtitle("Erizo Zona X Norte - Modelo alternativo")+
...
## 1.4. Análisis de sensibilidad a la Mortalidad natural - Modelo alternativo


```{r Dat_SenMortalidadNatural_XN_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/MortalidadNatural_XN_alt",sep="")
setwd(dir)
admb<-"MAETXNa"
...
```{r Fig_MortalidadNatural_XN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad de la Mortalidad natural de erizo de la zona norte. La línea negra y zona sombreada corresponde a caso base (Loo = 119.85 mm y M = 0.25 año-1)", fig.height=6, fig.width=6}
...
## 1.5. Análisis de sensibilidad a la Longitud asintótica - Modelo alternativo

```{r Dat_SenLoo_XN_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RangoLoo_XN_alt",sep="")
setwd(dir)
admb<-"MAETXNa"
...
```{r Fig_Loo_XN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad del rango de Loo de erizo de la zona norte. La línea negra y zona sombreada corresponde a caso base (Loo = 119.85 mm y M = 0.25 año-1)", fig.height=6, fig.width=6}
...
## 1.6. Variables de estado - modelo alternativo

```{r Fig_VarpoblXN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Variables de biomasas totales, desovantes, reclutamientos y desvíos estimadas por el modelo para el erizo de la zona X Norte período 1960 - 2019.", fig.height=6, fig.width=8}


Rt <- ggplot(VarPob) + 
    geom_line(aes(y=Rt1, x=x, colour = "Estimado Zona X Norte"), size=0.5)+
    geom_ribbon(aes(ymin=lowerRt1, ymax=upperRt1, x=x, fill = "IC"), alpha = 0.2)+
    labs(x = '', y = 'Reclutamientos',colour='Asesorías')  +
    scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 5)) +
    scale_colour_manual("",values=c("black"))+
    scale_fill_manual("",values=c("grey30"))+
...
```{r Fig_SelFlotaXN_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=5,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Selectividad de la flota de la Zona X Norte"}
...
## 1.7. Puntos Biológicos de Referencia - modelo alternativo

```{r Fig_PBRsXN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Puntos Biológicos de referencia de Erizo zona X Norte", fig.height=4.5, fig.width=4.5,eval=T}
...
## 1.8. Estatus del erizo de la zona norte de la Región de Los Lagos - modelo alternativo

```{r Fig_DiagramaFaseXN_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Diagrama de fase propuesto para erizo zona X Norte",fig.height=4.5, fig.width=4.5,eval=T}
...
## 2.1. Ajustes y residuos - modelo alternativo

```{r Fig_ajustesIndices_XS_alt, echo=FALSE,message=FALSE, warning=FALSE, fig.align="center",fig.path=dir.Fig, fig.height=5,  fig.width=5, dev=fig, include=T, fig.cap="Modelo alternativo. Ajuste del modelo a la información de CPUE, desembarque para el erizo de la zona X Sur. Los puntos representan a las observaciones junto a sus niveles de incertidumbre. La línea negra sólida muestra el valor estimado por el modelo"}
...
cpueXS <- ggplot(baseXS %>% filter(Asesoria!='observado', variable=='Biomasa'), 
       aes(yrs,value)) + 
       geom_line(aes(colour=Asesoria), size=0.8) +
       scale_colour_manual(values=c('black')) +
       geom_point(data = baseXS %>% filter(Asesoria=='observado', variable=='Biomasa'),
       aes(yrs,value), shape = 19, colour = 'gray30') +
       geom_errorbar(data = baseXS %>% filter(Asesoria=='observado', variable=='Biomasa'),
       aes(ymin = value*exp(-1.96*cvcpue), ymax = value*exp(1.96*cvcpue)), color = 'gray30') +
       scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 5)) +
       labs(x = '', y = ' toneladas') +
       theme_bw(base_size=9) + 
       ggtitle('Biomasa zona X Sur')+
       theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
```{r Fig_residuosIndices_XS_alt,warning=F, include=T, message=F, echo=F,fig.height=5,fig.width=7,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Residuos de la CPUE y desembarques de erizo de la zona X Sur"}
...
```{r ajustesCompFXS_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=8,fig.width=8,fig.align="center",fig.path=dir.Fig,dev=fig,  fig.cap="Modelo alternativo. Ajustes de la proporción de tallas de erizo de la zona X Sur"}
...
```{r Fig_residuosCompXS_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=6,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Residuos de la proporción de tallas de erizo de la zona X sur"}
...
## 2.2. Análisis retrospectivo de erizo zona X sur - modelo alternativo

```{r VariablesXS_std_alt, echo=F,message=FALSE, warning=FALSE, include=T}
...
```{r Dat_RetrospectivoXS_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RetrospectivoXS_alt",sep="")
setwd(dir)
admb<-"MAETXSa"
...
```{r Fig_RetrospectivoXS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Patrón retrospectivo estándar (panel izquierdo) y  relativo (panel derecho) de los reclutamientos", fig.path=dir.Fig,fig.height=6.5, fig.width=6}
...
## 2.3. Perfil de verosimilitud de erizo zona X sur - modelo alternativo

```{r Fig_VerosimilitudXS_alt,echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Perfil de verosimilitud erizo zona X sur", fig.height=5, fig.width=7}

source(paste(dir.fun,"functions.R",sep=""))
dir       <-paste(dir.0,"/VerosimilitudXS_alt",sep="")
setwd(dir)
...
for(i in 1:casos){
report      <- reptoRlist(paste(dir,"/MAETXSas",i,".rep",sep=""))
logRo[i]    <- report$Ro
likeval[i,] <- report$likeval}
...
names<-c("Ro","Biomasa","Desembarques","Prop_Flota","devRo","log_F","NA","NA","NA","NA","NA","Total")
# Tabla verosimilitud
TLk1 <- data.frame(exp(logRo),like);colnames(TLk1)<-names
# Tabla estandarizada
TLk2<- data.frame(exp(logRo),slikeval);colnames(TLk2)<-names
...
fig1<-ggplot() +
      ...
      xlab("Ro") + 
      ylab("L-min(L)") + 
     ggtitle("Erizo Zona X Sur - Modelo alternativo")+
     theme_bw(base_size=10) +
     theme(plot.title = element_text(hjust = 0.5),legend.position="left")
...
## 2.4. Análisis de sensibilidad a la mortalidad natural - modelo alternativo


```{r Dat_SenMortalidadNatural_XS_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/MortalidadNatural_XS_alt",sep="")
setwd(dir)
admb<-"MAETXSa"
...
```{r Fig_MortalidadNatural_XS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad de la Mortalidad natural de erizo de la zona X sur. La línea negra y zona sombreada corresponde a caso base (Loo = 119.85 mm y M = 0.282 año-1)", fig.height=6, fig.width=6}
...
## 2.5. Análisis de sensibilidad a la  Longitud asintótica - modelo alternativo

```{r Dat_SenLoo_XS_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RangoLoo_XS_alt",sep="")
setwd(dir)
admb<-"MAETXSa"
...
```{r Fig_Loo_XS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad del rango de Loo de erizo de la zona X sur. La línea negra y zona sombreada corresponde a caso base (Loo = 119.85 mm y M = 0.282 año-1)", fig.height=6, fig.width=6}
...
## 2.6. Variables de estado de erizo Zona X sur - modelo alternativo

```{r Fig_VarpoblXS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Variables poblacionales de Erizo zona X Sur", fig.path=dir.Fig,fig.height=6, fig.width=8}


Rt <- ggplot(VarPob) + 
    geom_line(aes(y=Rt2, x=x, colour = "Estimado Zona X sur"), size=0.5)+
    geom_ribbon(aes(ymin=lowerRt2, ymax=upperRt2, x=x, fill = "IC"), alpha = 0.2)+
    labs(x = '', y = 'Reclutamientos',colour='Asesorías')  +
    scale_x_continuous(breaks = seq(from = 1960, to = 2020, by = 5)) +
    scale_colour_manual("",values=c("black"))+
    scale_fill_manual("",values=c("grey30"))+
...
```{r Fig_SelFlotaXS_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=5,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Selectividad de la flota de la Zona X sur"}
# ASESORIA DE SEPTIEMBRE
age<-rep2$Edades
par(mfrow=c(1,1),mar=c(4,4,1,1))
...
## 2.7. Puntos Biológicos de Referencia - Modelo alternativo

```{r Fig_PBRsXS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Puntos Biológicos de referencia de Erizo zona X Sur", fig.height=4.5, fig.width=4.5,eval=T}
...
## 2.8. Estatus del erizo de la zona sur de la Región de Los Lagos - Modelo alternativo

```{r Fig_DiagramaFaseXS_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Diagrama de fase propuesto para erizo zona X Sur",fig.height=4.5, fig.width=4.5,eval=T}
...
## 3.1. Ajustes y residuos de erizo zona XI - Modelo alternativo

```{r Fig_ajustesIndices_XI_alt, echo=FALSE,message=FALSE, warning=FALSE, fig.align="center", fig.height=6, fig.path=dir.Fig, fig.width=6, dev=fig, include=T, fig.cap="Modelo alternativo. Ajuste del modelo a la información de CPUE, desembarque para el erizo de la zona X Sur. Los puntos representan a las observaciones junto a sus niveles de incertidumbre. La línea negra sólida muestra el valor estimado por el modelo"}
...
cpueXI <- ggplot(baseXI %>% filter(Asesoria!='observado', variable=='Biomasa'), 
       aes(yrs,value)) + 
       geom_line(aes(colour=Asesoria), size=0.8) +
       scale_colour_manual(values=c('black')) +
       geom_point(data = baseXI %>% filter(Asesoria=='observado', variable=='Biomasa'),
       aes(yrs,value), shape = 19, colour = 'gray30') +
       geom_errorbar(data = baseXI %>% filter(Asesoria=='observado', variable=='Biomasa'),
       aes(ymin = value*exp(-1.96*cvcpue), ymax = value*exp(1.96*cvcpue)), color = 'gray30') +
       scale_x_continuous(breaks = seq(from = 1960, to = 2021, by = 5)) +
       labs(x = '', y = 'toneladas') +
       theme_bw(base_size=9) + 
       ggtitle('Biomasa zona XI')+
       theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
```{r Fig_residuosIndices_XI_alt,warning=F, include=T, message=F, echo=F,fig.height=5,fig.width=7,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Residuos de la CPUE y desembarques de erizo de la zona XI"}
...
```{r ajustesCompFXI_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=8,fig.width=8,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Ajustes de la proporción de tallas de erizo de la zona XI"}
age  <-rep3$Tallas                                          
nage<-length(age)   
...
```{r Fig_residuosCompXI_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=6,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Residuos de la proporción de tallas de erizo de la zona XI"}
...
## 3.2. Análisis retrospectivo de erizo zona XI - Modelo alternativo

```{r VariablesXI_std_alt, echo=F,message=FALSE, warning=FALSE, include=T}
...
```{r Dat_RetrospectivoXI_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RetrospectivoXI_alt",sep="")
setwd(dir)
admb<-"MAETXIa"
...
```{r Fig_RetrospectivoXI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Patrón retrospectivo estándar (panel izquierdo) y  relativo (panel derecho) de los reclutamientos", fig.path=dir.Fig,fig.height=6.5, fig.width=6}
...
## 3.3. Perfil de verosimilitud de erizo zona XI - Modelo alternativo

```{r Fig_VerosimilitudXI_alt,echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Perfil de verosimilitud erizo zona XI", fig.height=5, fig.width=7}

source(paste(dir.fun,"functions.R",sep=""))

dir       <-paste(dir.0,"/VerosimilitudXI_alt",sep="")
setwd(dir)
...
for(i in 1:casos){
report      <- reptoRlist(paste(dir,"/MAETXIas",i,".rep",sep=""))
logRo[i]    <- report$Ro
likeval[i,] <- report$likeval}
...
fig1<-ggplot() +
      ...
      geom_hline(yintercept = 2,colour='black',lty=2) +
      scale_colour_manual("",values=c("green","blue","red","black"))+
      coord_cartesian(xlim = c(55,550), ylim = c(0, 3))+
      xlab("Ro") + 
      ylab("L-min(L)") + 
     ggtitle("Erizo Zona XI - Modelo alternativo")+
     theme_bw(base_size=10) +
     theme(plot.title = element_text(hjust = 0.5),legend.position="left")
...
## 3.4. Análisis de sensibilidad a la mortalidad natural de erizo zona XI - Modelo alternativo


```{r Dat_SenMortalidadNatural_XI_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/MortalidadNatural_XI_alt",sep="")
setwd(dir)
admb<-"MAETXIa"
...
```{r Fig_MortalidadNatural_XI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad de la Mortalidad natural de erizo de la zona XI. La línea negra y zona sombreada corresponde a caso base (Loo = 132.8 mm y M = 0.20 año-1)", fig.height=6, fig.width=6}
...
## 3.5. Análisis de sensibilidad a la Longitud asintótica - Modelo alternativo

```{r Dat_SenLoo_XI_alt, echo=F, message=FALSE, warning=FALSE, include=T}
dir<-paste(dir.0,"/RangoLoo_XI_alt",sep="")
setwd(dir)
admb<-"MAETXIa"
...
```{r Fig_Loo_XI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Análisis de sensibilidad del rango de Loo de erizo de la zona XI. La línea negra y zona sombreada corresponde a caso base (Loo = 132.8 mm y M = 0.20 año-1)", fig.height=6, fig.width=6}
...
## 3.6. Variables de estado de erizo zona XI - Modelo alternativo

```{r Fig_VarpoblXI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Variables poblacionales de Erizo zona XI", fig.path=dir.Fig,fig.height=6, fig.width=8}


Rt <- ggplot(VarPob) + 
    geom_line(aes(y=Rt3, x=x, colour = "Estimado Zona XI"), size=0.5)+
    geom_ribbon(aes(ymin=lowerRt3, ymax=upperRt3, x=x, fill = "IC"), alpha = 0.2)+
    labs(x = '', y = 'Reclutamientos',colour='Asesorías')  +
...
BD <- ggplot(VarPob) + 
     ...
     geom_ribbon(aes(ymin=lowerBD3, ymax=upperBD3, x=x, fill = "IC"), alpha = 0.2)+
    labs(x = '', y = 'Biomasa desovante',colour='Asesorías')  +
    scale_x_continuous(breaks = seq(from = years[1], to = years[nyears], by = 5)) +
    scale_colour_manual("",values=c("black"))+
    scale_fill_manual("",values=c("grey30"))+
...
```{r Fig_SelFlotaXI_alt,warning=F, include=T, message=F, echo=FALSE,fig.height=5,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig, fig.cap="Modelo alternativo. Selectividad de la flota de la Zona XI"}
...
## 3.7. Puntos Biológicos de Referencia de erizo zona XI - Modelo alternativo

```{r  Fig_PBRsXI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Puntos Biológicos de referencia de Erizo zona XI", fig.height=4.5, fig.width=4.5,eval=T}
...
## 3.8. Estatus del erizo de la Región de Aysén - Modelo alternativo

```{r Fig_DiagramaFaseXI_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Modelo alternativo. Diagrama de fase propuesto para erizo zona XI",fig.height=4.5, fig.width=4.5,eval=T}
...
# 4. Análisis integrado de las tres zonas de estudio - Modelo alternativo

```{r datInd2_alt, warning=F, include=T, message=F, echo=FALSE,fig.height=6,fig.width=5,fig.align="center",fig.path=dir.Fig,dev=fig,eval=T}
...
```{r Plot_Biomasas_alt, echo=FALSE, message=FALSE, warning=FALSE, include=T, fig.align="center", fig.cap="Biomasas Totales y Desovantes"}
