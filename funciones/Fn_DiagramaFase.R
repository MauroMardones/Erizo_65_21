...
#********************************************************
DiagramaFase<-function(name,FRMS,BRMS,SpB,SpBSE,ln_Fyr,ln_FSE,Year,col_lastyear,ylims,xlims){
...
BlimC    <- 0         #l?mite de colapso o agotamiento
BTargC   <- 1.0                #Brms
FTarg    <- 1.0                #Frms
#-------------------------------------------------------------------------------------------------
maxaxes  <- length(Year)
ifelse(length(maxaxes)>1,xupdown<-c(0,maxaxes[1]),xupdown<-c(0,xlims))
ifelse(length(maxaxes)>1,yupdown<-c(0,maxaxes[3]),yupdown<-c(0,ylims))
#-------------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------
leftFE   <- 0.875    # l?mite izquierdo de plenaExplotaci?n
rightFE  <- 1.125    # l?mite derecho de plenaExplotaci?n 
upFE     <- 1.125    # l?mite superior de plenaExplotaci?n
downFE   <- 0.875    # l?mite inferior de plenaExplotaci?n
#-----------------------------------------------------------------
...
plot(Bval,Fval,type="n",pch=16,cex=0.8,lwd=1,xlim=c(0,xlims),xaxs="i",
       ylim=c(0,ylims),yaxs="i",xlab="",ylab="")
#----------------------------------
...
if (upspace > 0) mtext(name,side=3,outer=T,cex=1,font=usefont)
  ...
  arrows(x0=B95[1],y0=lastF,x1=B95[2],y1=lastF,length=0.05,angle=90,col=col_lastyear,lwd=1,code=3,lty=1)
  arrows(x0=lastB,y0=F95[1],x1=lastB,y1=F95[2],length=0.05,angle=90,col=col_lastyear,lwd=1,code=3,lty=1)
